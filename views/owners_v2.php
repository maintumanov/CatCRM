        ...
        <div class="col-md-8">
            <div class="md-card p-4">
                ...
                <div class="mb-4 p-3 bg-light rounded">
                    ...
                    <?php if ($cat): ?>
                        <a href="?action=cat&id=<?= $cat['id'] ?>" class="text-decoration-none fw-medium">
                ...
                <!-- 🔥 ЗАПОЛНЕННАЯ JSON-АНКЕТА -->
                <div class="mb-4">
                    <h6 class="fw-bold mb-3 border-bottom pb-2">📝 Заполненная анкета (из формы)</h6>
                    <?php
                    $app_data = $owner['application_result'] ?? null;
                    // Поддержка: если данные пришли как строка (JSON), декодируем
                    if (is_string($app_data)) {
                        $app_data = json_decode($app_data, true);
                        // Если это путь к файлу, пробуем загрузить его
                        if (!$app_data && file_exists($app_data)) {
                            $app_data = json_decode(file_get_contents($app_data), true);
                        }
                    }
                    
                    if (!$app_data || empty($app_data)): ?>
                        <div class="alert alert-light text-muted fst-italic">Данные анкеты отсутствуют или не были сохранены</div>
                    <?php else: ?>
                        <div class="p-3 bg-light rounded border" style="white-space: pre-line;">
                            <?= nl2br(escape(formatApplicationResult($app_data))) ?>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="mb-4">
                    <h6 class="fw-bold mb-3 border-bottom pb-2">📝 Заметки куратора</h6>
            ...
            <div class="md-card p-4">
                ...
                <ul class="list-unstyled mb-0 small">
                    ...
                    <li class="mb-2 pb-2 border-bottom">
                        ...
                        <small class="text-muted"><?= date('d.m.Y H:i', strtotime($h['date'] ?? 'now')) ?></small>
                    </li>
                    <?php endforeach; ?>
                </ul>
            </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr><td colspan="6" class="text-center text-muted py-4">Нет записей</td></tr>
                    ...
                    <tr>
                        <td>
                            ...
                            <?php if (!empty($o['email'])): ?><small class="text-muted"><?= escape($o['email']) ?></small><?php endif; ?>
                        </td>
                        <td><?= escape($o['phone']) ?></td>
                        <td>
                            <?php if ($cat): ?>
                                <a href="?action=cat&id=<?= $cat['id'] ?>" class="text-decoration-none"><?= escape($cat['identification']['name']) ?></a>
                            <?php else: ?>
